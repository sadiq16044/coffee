...
function Circlebar(prefs) {
    ...
    this.initialise = function() {
        ...
        that.dialWidth = 30; //now default width of circle border is 30px
                ...
                var internalCallback = function(counter) {
                    return function() {
                        if (that.value < that.maxValue && that.value < 100) {
                            ...
                            setTimeout(internalCallback, counter * factor); // for 9 mins
                        }
                    }
                }(times, 0);
                setTimeout(internalCallback, 1 * 1000); // for 9 mins
            };
            setDeceleratingTimeout(1, 100);
        }
    }
    this.textFilter();
